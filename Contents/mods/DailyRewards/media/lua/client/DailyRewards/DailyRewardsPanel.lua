...
local function buildDailyTooltip(row)
	...
	local title = row.reward and row.reward.title
		or (getText("Tooltip_DR_Day") .. " " .. tostring(row.dayNumber or "?"))
...
function DailyRewardsPanel:createChildren()
	...
	self.streakList =
		ISScrollingListBox:new(C.LAYOUT.PAD, self.dailyGrid:getBottom() + C.PLAYER.SECTION_GAP, leftW, 108)
	...
	self.claimDailyBtn = addButton(
		self,
		C.LAYOUT.PAD,
		actionY,
		C.CTRL.CLAIM_DAILY_W,
		getText("IGUI_DR_ClaimDay"),
		"CLAIM_DAILY",
		"primary"
	)
	self.claimStreakBtn = addButton(
		self,
...
function DailyRewardsPanel:createAdminChildren(contentY)
	...
	self.settingsTickBox = ISTickBox:new(
		C.LAYOUT.PAD,
		settingsY,
		C.SETTINGS.BOX_W,
		C.SETTINGS.BOX_H,
	...
	self.startDatePickBtn = addButton(
		self,
	...
	self.resetStreakEntry =
		addEntry(self, rightSettingsX, fieldY + C.SETTINGS.ROW_GAP, C.SETTINGS.VALUE_W, C.CTRL.FIELD_H)
	...
	self.adminStreakModeBtn = addButton(
		self,
	...
	self.deleteRewardBtn = addButton(
		self,
	...
	self.rewardTitleEntry = addEntry(
		self,
	...
	self.rewardRepeatTickBox = ISTickBox:new(
	...
	self.addRewardItemBtn = addButton(
		self,
	...
	self.removeRewardItemBtn = addButton(
		self,
	...
	self.rewardItemList = addDataList(
		self,
		editorX,
	...
	self.rewardXpAmountEntry =
		addEntry(self, self.rewardXpSkillCombo:getRight() + C.LAYOUT.GAP_M, xpRowY, 50, C.CTRL.FIELD_H)
	self.rewardXpAmountEntry:setOnlyNumbers(true)
	self.addRewardXpBtn = addButton(
		self,
	...
	self.removeRewardXpBtn = addButton(
		self,
	...
	self.addRewardTraitBtn = addButton(
		self,
	...
	self.removeRewardTraitBtn = addButton(
		self,
	...
	self.rewardTraitList = addDataList(
		self,
		editorX,
	...
	self.importConfigBtn = addButton(
		self,
	...
	self.exportConfigBtn = addButton(
		self,
...
function DailyRewardsPanel:layoutChildren(force)
	...
	local adminListW = narrowAdmin and C.ADMIN.LIST_W_NARROW
		or (compactAdmin and C.ADMIN.LIST_W_COMPACT or C.ADMIN.LIST_W)
	...
	if self.adminMode == "streak" then
		setBounds(
			self.rewardRepeatTickBox,
			self.rewardTitleEntry:getRight() + C.LAYOUT.GAP,
			editorY,
			repeatW,
			C.CTRL.FIELD_H
		)
	...
	setBounds(
		self.rewardItemCountEntry,
		self.rewardItemTypeEntry:getRight() + controlGap,
		itemRowY,
		countW,
		C.CTRL.FIELD_H
	)
	...
	setBounds(
		self.removeRewardItemBtn,
		self.addRewardItemBtn:getRight() + controlGap,
		itemRowY,
		removeW,
		C.CTRL.BUTTON_H
	)
	...
	setBounds(
		self.rewardXpAmountEntry,
		self.rewardXpSkillCombo:getRight() + controlGap,
		xpRowY,
		amountW,
		C.CTRL.FIELD_H
	)
	...
	setBounds(
		self.removeRewardTraitBtn,
		self.addRewardTraitBtn:getRight() + controlGap,
		traitRowY,
		removeW,
		C.CTRL.BUTTON_H
	)
...
function DailyRewardsPanel:refreshRewardItemList()
	...
	if #self.rewardItemsData == 0 then
		...
		self.selectedRewardItemIndex = math.max(
			1,
...
function DailyRewardsPanel:refreshRewardXpList()
	...
	if #self.rewardXpData == 0 then
		...
		self.selectedRewardXpIndex = math.max(
			1,
...
function DailyRewardsPanel:refreshRewardTraitList()
	...
	if #self.rewardTraitsData == 0 then
		...
		self.selectedRewardTraitIndex = math.max(
			1,
			math.min(self.selectedRewardTraitIndex > 0 and self.selectedRewardTraitIndex or 1, #self.rewardTraitsData)
		)
	end
...
function DailyRewardsPanel:addRewardXp()
	local perkType = self.rewardXpSkillCombo and self.rewardXpSkillCombo:getOptionData(self.rewardXpSkillCombo.selected)
		or nil
...
function DailyRewardsPanel:addRewardTrait()
	local traitType = self.rewardTraitCombo and self.rewardTraitCombo:getOptionData(self.rewardTraitCombo.selected)
		or nil
...
function DailyRewardsPanel:populateAdminRewardsList()
	...
	for i = 1, #rewards do
		self.adminRewardsList:addItem(tostring(rewards[i].title or rewards[i].id or i), {
			index = i,
			mode = self.adminMode,
			entry = rewards[i],
		})
...
function DailyRewardsPanel:saveAdminEditorToSelected()
	...
	local data = buildRewardConfig(getEntryText(self.rewardTitleEntry), getEntryText(self.rewardDescriptionEntry), {
...
function DailyRewardsPanel:openStartDatePicker()
	...
	local initial = dateKeyToDateTable(getEntryText(self.startDateEntry))
		or dateKeyToDateTable(self.snapshot and self.snapshot.todayKey)
		or dateKeyToDateTable(Shared.GetTodayKey())

	local picker = DateTimeSelector:new(0, 0, 0, 0, false, initial, {
		showTime = false,
...
function DailyRewardsPanel.drawRewardItemListItem(list, y, item, alt)
	...
	drawClippedListRow(list, y, list.itemheight, function()
		list:drawRect(
			0,
			y,
			list:getWidth(),
		...
		drawRewardIconTile(list, {
			type = "item",
			texture = getItemTexture(row.type),
			fallback = "IT",
		}, C.LIST.ICON_PAD, iconY, iconSize)
		list:drawText(
...
function DailyRewardsPanel.onRewardComboChanged(target, combo) end
...
local function getDailyTileMetrics(list)
	local columns = list.dailyRewardColumns
		or (list.parentPanel and list.parentPanel.dailyGridColumns)
		or C.GRID.COLUMNS
...
function DailyRewardsPanel.drawStreakItem(list, y, item, alt)
	...
	local selected = list.selected == item.index
	local background = selected and C.COLORS.SELECTED or (alt and C.COLORS.ALT or C.COLORS.FIELD)
	local statusText = row.claimed and getText("IGUI_DR_Claimed")
		or (
			row.claimable and getText("IGUI_DR_Ready")
			or tostring(row.progress or 0) .. "/" .. tostring(row.requiredDays or 0)
		)
...
function DailyRewardsPanel.drawAdminRewardItem(list, y, item, alt)
	...
	local selected = list.selected == item.index
	local background = selected and C.COLORS.SELECTED or (alt and C.COLORS.ALT or C.COLORS.FIELD)
	local numberText = row.mode == "streak" and (tostring(entry.days or "?") .. " " .. getText("IGUI_DR_Days"))
		or ("Day " .. tostring(entry.day or row.index))
...
function DailyRewardsPanel:drawPlayerDetails()
	...
	if streakRow and streakRow.streak and y < rect.y + rect.height - 70 then
		local status = streakRow.claimed and getText("IGUI_DR_Claimed")
			or (
				streakRow.claimable and getText("IGUI_DR_Ready")
				or tostring(streakRow.progress or 0) .. "/" .. tostring(streakRow.requiredDays or 0)
			)
		self:drawText(
...
function DailyRewardsPanel:drawAdminHints()
	...
	local settingsLabelW = self._settingsLabelW
		or (self._narrowAdminLayout and C.SETTINGS.LABEL_W_NARROW or C.SETTINGS.LABEL_W)
	...
	local sectionY = self._adminSectionLabelY
		or (self.adminRewardsList:getY() - FONT_HGT_SMALL - C.ADMIN.SECTION_TITLE_GAP)
	self:drawText(
