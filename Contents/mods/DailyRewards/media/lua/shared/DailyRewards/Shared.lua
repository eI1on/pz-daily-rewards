...
function Shared.NormalizeConfig(config)
	...
	local out = {
		...
		resetStreakAfterMissedDays = math.floor(
			normalizeNumber(config.resetStreakAfterMissedDays, defaults.resetStreakAfterMissedDays, 0, 3650)
		),
		dailyRewards = {},
		streakRewards = {},
	}

	local dailyRewards = type(config.dailyRewards) == "table" and #config.dailyRewards > 0 and config.dailyRewards
		or defaults.dailyRewards
...
function Shared.SaveConfigToFile(config)
	...
	local success =
		FileUtils.writeJson(DailyRewards.CONFIG_FILE, normalized, DailyRewards.FILE_MOD_ID, { createIfNull = true })
	if success then
...
function Shared.GetPlayerState(data, player, todayKey)
	...
	state.firstSeenAt = tonumber(state.firstSeenAt)
		or Shared.DateKeyToTimestamp(todayKey)
		or Shared.GetServerTimestamp()
...
function Shared.GetCampaignStartDate(config, state)
	return Shared.NormalizeDateKey(config and config.startDate)
		or Shared.EpochToDateKey(state and state.firstSeenAt)
		or Shared.GetTodayKey()
end
...
function Shared.RebuildStreakState(state, todayKey, resetStreakAfterMissedDays)
	...
	for i = 1, #keys do
		...
		local claimEpoch = tonumber(claimValue)
			or Shared.DateKeyToTimestamp(claimValue)
			or Shared.DateKeyToTimestamp(rewardDateKey)
			or Shared.GetServerTimestamp()
...
function Shared.ApplyDailyClaimToStreak(state, rewardDateKey, claimDateKey, resetStreakAfterMissedDays)
	if state then
		...
		if rewardDateKey then
			state.claimedDaily = type(state.claimedDaily) == "table" and state.claimedDaily or {}
			local claimEpoch = tonumber(claimDateKey)
				or Shared.DateKeyToTimestamp(claimDateKey)
				or Shared.GetServerTimestamp()
...
function Shared.GetRewardSummary(rewards)
	...
	for i = 1, #rewards.traits do
		...
		parts[#parts + 1] =
			string.format("Trait: %s", traitInfo and traitInfo.label or tostring(rewards.traits[i].type or "Trait"))
	end
